...
It emphasizes the use of interfaces for behavior contracts while forcing infrastructure to be externalized. This leads to the separation of different infrastructure problems from actual business requirements.
...
- CogentCode.Template.Application
- CogentCode.Template.Application.Contracts

### CogentCode.Template.Application.Contracts

This is where we are defining contracts our **CogentCode.Template.Application** Layer needs in order to communicate with Infrastructure layer.

**This layer only includes models and interfaces.**

### CogentCode.Template.Application

This is where all business logic should happen. We are doing this by leveraging **MediatR** commands and queries, which nicely adhere to the **Separation of Concerns Principle**. In order to communicate with database or any other system, we are using interfaces we define in **CogentCode.Template.Application.Contracts** to achieve this.
...
- CogentCode.Template.Infrastructure.Db -> You need to have something shared between all db libraries
- CogentCode.Template.Infrastructure.Db.Mssql -> you need something shared between First and Second
- CogentCode.Template.Infrastructure.Db.Mssql.First
- CogentCode.Template.Infrastructure.Db.Mssql.Second
- CogentCode.Template.Infrastructure.Db.Postgres.Third
- CogentCode.Template.Infrastructure.Http.Four

This helps to keep your dependencies clean.

## Presentation Layer

Entry layer of application. Suitable for REST, GRPC, GRAPHQL entry to application.
...
``` bash
  dotnet new install .nupkg\cogentcode.template.api.1.0.0.nupkg
```
...
``` bash
  nuget pack cogentcode-template.nuspec -OutputDirectory .nupkg" -NoDefaultExcludes
```
